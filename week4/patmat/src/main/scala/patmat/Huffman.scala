...
object Huffman {
   ...
   */
    def times(chars: List[Char]): List[(Char, Int)] = {
      def timesAcc(chars: List[Char], acc: List[(Char, Int)]): List[(Char, Int)] = {
        chars match {
          case head :: tail => timesAcc(tail, accumulate(head, acc))
          case Nil => acc
        }
      }
      def accumulate(x: Char, acc: List[(Char, Int)]): List[(Char, Int)] = {
        acc match {
          case (char, times) :: tail =>
            if (x == char) (char, times + 1) :: tail
            else (char, times) :: accumulate(x, tail)
          case Nil => (x, 1) :: Nil
        }
      }
      timesAcc(chars, Nil)
  ...
  def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = {
    freqs.map(freqs => Leaf(freqs._1, freqs._2)).sortBy(leaf => leaf.weight)
  }
   ...
   */
  def until(predicate: List[CodeTree] => Boolean, perform: List[CodeTree] => List[CodeTree])(trees: List[CodeTree]): CodeTree = {
    if (!predicate(trees))
      until(predicate, perform)(perform(trees))
    else
      trees.head
  }
    ...
    def decodeRest(branch: CodeTree, bits: List[Bit], decoded: List[Char]): List[Char] =
      branch match {
        ...
        case Fork(left, right, _, _) =>
          bits match {
            case Nil => decoded
            case 0 :: tail => decodeRest(left, tail, decoded)
            case 1 :: tail => decodeRest(right, tail, decoded)
            case x :: xs => throw new IllegalStateException("unknown bit value")
          }
      }
  ...
  def codeBits(table: CodeTable)(char: Char): List[Bit] =
    table match {
      case Nil => throw new NoSuchElementException("no code for char: " + char)
      case head :: tail =>
        if (head._1 == char)
          head._2
        else
  ...
  def convert(tree: CodeTree): CodeTable = {
    def traverse(tree: CodeTree, code: List[Bit]): CodeTable =
      tree match {
        case Leaf(char, _) => List((char, code))
        case Fork(left, right, _, _) => mergeCodeTables(
          traverse(left, code ::: List(0)),
          traverse(right, code ::: List(1))
        )
      }
    traverse(tree, Nil)
  }
   ...
   */
    def quickEncode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def getCode: Char => List[Bit] = codeBits(convert(tree))
      def encodeRest(text: List[Char], code: List[Bit]): List[Bit] = {
        text match {
          case Nil => code
          case char :: restOfTheText =>
            encodeRest(restOfTheText, code ::: getCode(char))
        }
      }
      encodeRest(text, Nil)
    }
